class PlayerAbility:
    ...
    def getAbility(self):
        return self.abiliity
    def getTechnical(self):
        return self.ability["테크닉"]
    def getMental(self):
        return self.ability["멘탈"]
    def getPhysical(self):
        return self.ability["피지컬"]
    def getGoalkeeping(self):
        return self.ability["골키퍼능력"]
    def getPotential(self):
        return self.ability["잠재력"]

    def setTechnical(self, key, val):
        if key in self.ability["테크닉"]:
            if val >= 0 and val <= 99:
                self.ability["테크닉"][key] = val
    def setMental(self, key, val):
        if key in self.ability["멘탈"]:
            if val >= 0 and val <= 99:
                self.ability["멘탈"][key] = val
    def setPhysical(self, key, val):
        if key in self.ability["피지컬"]:
            if val >= 0 and val <= 99:
                self.ability["피지컬"][key] = val
    def setGoalKeeping(self, key, val):
        if key in self.ability["골키퍼능력"]:
            if val >= 0 and val <= 99:
                self.ability["골키퍼능력"][key] = val
    def setPotential(self, val):
        if val >= 0 and val <= 99:
            self.ability["잠재력"] = val
